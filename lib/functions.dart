...
void sendNewRecipePost(String recipeName,
  ...
  try {
    ...
    String encryptedCodeId = encrypter
        .encrypt(codeId)
        .base64;
...
Future<String> sendLoginPostRequest(String userName, String password) async {
  try{
    Encrypter encrypter = await getEncrypter();
    String encrypted = encrypter
        .encrypt(password)
        .base64;

    http.Response loginResponse = await http.post(Uri.http("$IP:$PORT", "/login"),
        headers: <String, String>{
          'Content-Type': 'application/json; charset=UTF-8',
        },
